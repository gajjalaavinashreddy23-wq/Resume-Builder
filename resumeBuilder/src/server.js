...
if (!MONGODB_URI) {
  console.warn(
    "⚠️ MONGODB_URI is not set. Set it in your environment or in a .env file.",
  );
}
...
app.post("/generate-resume", requireAuth, async (req, res) => {
  try {
    ...
    const safeName =
      `${originalData.firstName || "user"}_${originalData.lastName || "resume"}`
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase();
...
if (!PDFLATEX) {
  console.error(
    'PDFLATEX_PATH environment variable is not set. Defaulting to "pdflatex".',
  );
}
