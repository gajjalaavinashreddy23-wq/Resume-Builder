...
async function callAI(prompt, isJSON = false) {
  try {
    // Use environment variable for the OpenRouter API key
...
`;
          ...
          try {
            ...
            console.error(
              "Error parsing AI response for experience:",
              jsonError,
            );
      ...
      const enhanceWithRetry = async (fn, ...args) => {
        try {
          ...
          try {
            ...
            console.error(
              `Enhancement failed after retry: ${retryError.message}`,
            );
